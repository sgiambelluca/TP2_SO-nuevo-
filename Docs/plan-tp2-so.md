...
- [ ] **PENDIENTE:** `test_mm` debe ejecutarse como **proceso separado** (no built-in de la shell). Ver Paso 5.4.
...
### 2.8 Tests
- [ ] **PENDIENTE:** `test_proc` como proceso separado (foreground y background). Ver Paso 5.4.
- [ ] **PENDIENTE:** `test_prio` como proceso separado. Ver Paso 5.4.
...
### 3.3 Test
- [ ] **PENDIENTE:** `test_sync` como proceso separado. Ver Paso 5.4.
...
- [ ] Estructura del pipe:
  ...
  - Semaforo `sem_not_empty` (para bloquear lector cuando vacio).
  - Semaforo `sem_not_full` (para bloquear escritor cuando lleno).
  - Contador de extremos abiertos (para detectar EOF).
- [ ] Lectura bloqueante: si el pipe esta vacio, el lector se bloquea (sin busy waiting).
- [ ] Escritura bloqueante: si el pipe esta lleno, el escritor se bloquea.
- [ ] **EOF**: cuando se cierra el extremo de escritura (contador de escritores llega a 0), `read` retorna 0 en vez de bloquearse.
- [ ] Tabla global de pipes (anonimos y nombrados).

### 4.2 Abstraccion de file descriptors (FD)

**Este es el punto mas critico del TP.** Hace que `sys_read`/`sys_write` sean transparentes.

- [ ] Definir una union/struct de FD que pueda apuntar a:
  - Terminal (teclado/pantalla — comportamiento actual).
  - Extremo de lectura de un pipe.
  - Extremo de escritura de un pipe.
- [ ] Cada PCB tiene `fd[2]`: `fd[0]` = stdin, `fd[1]` = stdout.
- [ ] Modificar `sys_read` para consultar `current_process->fd[0]` y despachar al terminal o al pipe.
- [ ] Modificar `sys_write` para consultar `current_process->fd[1]` y despachar al terminal o al pipe.
- [ ] **Ningun proceso necesita saber si lee/escribe de un pipe o de la terminal** — la transparencia es un requisito del enunciado.
...
- [ ] Verificar que `Userland/c/include/syscall.h` expone la API `my_*` exactamente como la espera `https://github.com/alejoaquili/ITBA-72.11-SO/blob/main/kernel-development/tests/syscall.h`.
- [ ] `test_mm <max_bytes>` — proceso separado, foreground y background.
- [ ] `test_proc <max_procs>` — proceso separado, foreground y background.
- [ ] `test_prio <target_value>` — proceso separado.
- [ ] `test_sync <pairs> <increments> <use_sem>` — proceso separado, foreground y background.
...
[HECHO] Paso 3 (Semaforos)
   ...
   +-------+--------+
   |                |
Paso 4.1-4.2    Paso 5.2 (comandos simples: help, mem, ps, loop, kill, nice, block)
(Pipes + FDs)       |
   |                |
...
**Prioridad inmediata:**
1. **Paso 4.2 (FD abstraction)** — es el core de los pipes y habilita todo lo demas.
2. **Paso 4.1 (buffer circular + bloqueo)** — en paralelo con 4.2.
3. **Paso 5.4 (tests como procesos)** — se puede hacer sin pipes si la shell soporta crear procesos.
4. **Paso 5.1 (shell con & y waitpid)** — necesario para poder demostrar foreground/background.
